    ...
    <main className="mx-auto max-w-7xl px-6 py-12 lg:px-8">
      <section className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div className="space-y-6">
          <p className="inline-flex rounded-full bg-violet-50 px-4 py-1 text-sm font-semibold uppercase tracking-[0.28em] text-violet-700">
            Parampara</p>
          ...
          <div className="flex flex-col gap-4 sm:flex-row">
            ...
            <Link href="#impact" className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-6 py-3 text-base font-semibold text-slate-700 transition hover:bg-slate-50">
              Learn more
            </Link>
          </div>
        </div>
        <div className="rounded-[2rem] bg-gradient-to-br from-violet-700 to-fuchsia-500 p-1 shadow-2xl shadow-violet-500/20">
          <div className="rounded-[2rem] bg-white p-8">
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-violet-700">Featured instructors</p>
            <div className="mt-6 space-y-4">
              {instructors.slice(0, 3).map((instructor) => (
                <div key={instructor.id} className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                  <p className="text-base font-semibold text-slate-900">{instructor.name}</p>
                  <p className="mt-1 text-sm text-slate-600">{instructor.expertise}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="mt-20 grid gap-8 lg:grid-cols-3">
        <article className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          <h2 className="text-xl font-semibold text-slate-900">For learners</h2>
          <p className="mt-4 text-slate-600">Discover a trusted cultural space for your child, family member, or yourself to relearn language, festivals, and home traditions.</p>
        </article>
        <article className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          <h2 className="text-xl font-semibold text-slate-900">For women instructors</h2>
          <p className="mt-4 text-slate-600">Offer private teaching sessions on your schedule, share your skills, and earn hourly income from a global audience.</p>
        </article>
        <article className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          <h2 className="text-xl font-semibold text-slate-900">Real cultural bridge</h2>
          <p className="mt-4 text-slate-600">Build meaningful relationships that preserve heritage while creating economic independence for Indian women.</p>
        </article>
      </section>
      ...
      <section className="mt-24">
        <div className="flex items-center justify-between gap-4 rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          <div>
            ...
            <p className="mt-2 text-slate-600">Browse instructors, choose a time, and start a learning journey rooted in authentic Indian heritage.</p>
          </div>
          <Link href="/instructors" className="rounded-full bg-violet-700 px-6 py-3 text-sm font-semibold text-white transition hover:bg-violet-800">Explore instructors</Link>
        </div>
      </section>
    </main>
  );
